 ...
 * <p>
 * 64位ID (42(毫秒)+7(机器ID)+3(随机编码)+12(重复累加))
 */
...
func (s *Snowflake) GetId() (int64, int64) {
	...
	for _, netInterface := range network {
		if ip, ok := netInterface.(*net.IPNet); (ok && !ip.IP.IsLoopback()) || len(network) == 0 {
			s.masterId = 1
			sId += 1
			s.masterId = int64(sId)
			return s.masterId, int64(ipint)
		}
...
func retureSnowflake(a, b int64) *Snowflake {
	return &Snowflake{
		slaverId: a,
		masterId: b,
	}
...
func BenchmarkXxx(b *testing.B) {
	Snowflakes := retureSnowflake(7, 155)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		Snowflakes.GetId()
		_ = Snowflakes.NextVal()
	}
}
